...
<div class="max-w-4xl">
    ...
    <form method="post" action="{{ route('admin.pages.hero-slideshow.update', $page) }}" enctype="multipart/form-data" class="mt-8 space-y-6">
        @csrf
        @method('PUT')
        @include('admin.pages._blocks-home-hero-slideshow-form', ['blocks' => $blocks ?? []])

        <button type="submit" class="rounded-full bg-lake-900 px-6 py-2 text-sm font-semibold text-white">{{ __('Save slideshow') }}</button>
    </form>
</div>
@endsection
